...
void set_led_wrapper(safe_led& led_object, int x, int y, Colorcode color) {
    led_object.set_led(x, y, color);
}
...
void display_image(uint64_t image, safe_led& led_object) {
    for (int y = 0; y < 8; y++) {
        for (int x = 0; x < 8; x++) {
            int bit_index = y * 8 + x;
...
void image_loop(safe_led& led_object) {
    static int i = 0;
    display_image(IMAGES[i%2], led_object);
    i++;
}

void make_pattern(safe_led& led_object) {
